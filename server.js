...
app.get('/team_stats', function(req, res) {
    ...
    .then(data => {
        res.render('pages/team_stats.ejs',{
                my_title: "team_stats",
                result_1: data[0],
                result_2: data[1],
                result_3: data[2],
...
app.get('/player_info', function(req, res) {
	...
	db.any(query)
        .then(function (rows) {
            res.render('pages/player_info.ejs',{
        ...
        .catch(function (err) {
            // display error message in case an error
            console.log('error', err);
            response.render('pages/player_info.ejs', {
                title: 'player_info',
                data: '',
...
app.post('/player_info/select_player', function(req, res) {
    ...
    var query2 = 'select * from football_players where id ='+ req.body.id+' ;';
    var query3 = 'select * from football_players where id ='+ req.body.id+' ;';
	db.task('get-everything', task => {
        return task.batch([
            task.any(query1),
            task.any(query2),
